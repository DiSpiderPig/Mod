...
public class BlackKatanaUse extends AbstractWindCharge {
    private static final ExplosionDamageCalculator EXPLOSION_DAMAGE_CALCULATOR = new SimpleExplosionDamageCalculator(
            true, true, Optional.of(1f), BuiltInRegistries.BLOCK.getTag(BlockTags.ICE).map(Function.identity())
    );
    public static float RADIUS = 1f;

    private int noDeflectTicks = 5;

    public BlackKatanaUse(EntityType<? extends AbstractWindCharge> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public BlackKatanaUse(Player pPlayer, Level pLevel, double pX, double pY, double pZ) {
