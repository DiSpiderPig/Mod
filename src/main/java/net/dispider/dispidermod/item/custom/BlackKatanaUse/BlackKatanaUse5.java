...
public class BlackKatanaUse5 extends AbstractWindCharge {
    private static final ExplosionDamageCalculator EXPLOSION_DAMAGE_CALCULATOR = new SimpleExplosionDamageCalculator(
            true, true, Optional.of(5f), BuiltInRegistries.BLOCK.getTag(BlockTags.ICE).map(Function.identity())
    );
    public static float RADIUS = 5f;

    private int noDeflectTicks = 5;

    public BlackKatanaUse5(EntityType<? extends AbstractWindCharge> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public BlackKatanaUse5(Player pPlayer, Level pLevel, double pX, double pY, double pZ) {
