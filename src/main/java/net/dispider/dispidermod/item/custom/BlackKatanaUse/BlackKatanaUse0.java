...
public class BlackKatanaUse0 extends AbstractWindCharge {
    private static final ExplosionDamageCalculator EXPLOSION_DAMAGE_CALCULATOR = new SimpleExplosionDamageCalculator(
            true, true, Optional.of(0f), BuiltInRegistries.BLOCK.getTag(BlockTags.ICE).map(Function.identity())
    );
    public static float RADIUS = 0f;

    private int noDeflectTicks = 5;

    public BlackKatanaUse0(EntityType<? extends AbstractWindCharge> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public BlackKatanaUse0(Player pPlayer, Level pLevel, double pX, double pY, double pZ) {
