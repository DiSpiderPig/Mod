...
public class BlackKatanaUse3 extends AbstractWindCharge {
    private static final ExplosionDamageCalculator EXPLOSION_DAMAGE_CALCULATOR = new SimpleExplosionDamageCalculator(
            true, true, Optional.of(3f), BuiltInRegistries.BLOCK.getTag(BlockTags.ICE).map(Function.identity())
    );
    public static float RADIUS = 3f;

    private int noDeflectTicks = 5;

    public BlackKatanaUse3(EntityType<? extends AbstractWindCharge> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public BlackKatanaUse3(Player pPlayer, Level pLevel, double pX, double pY, double pZ) {
